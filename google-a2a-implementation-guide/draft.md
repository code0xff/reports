...
### 2.2 2026-09-22 재검증: 스펙은 멈추고 SDK와 재단이 움직였다

11일 사이의 변화는 한 문장으로 요약된다. **스펙은 한 줄도 바뀌지 않았고, 그 아래위가 움직였다.**

**스펙: v1.0.1 그대로, 그러나 대기열에 보안 수정이 있다.** 릴리스 페이지의 최신 태그는 여전히 2026년 5월 28일의 v1.0.1이다[^s30]. 정지된 것처럼 보이지만 저장소에는 열린 PR이 62건 있고, 그중 둘은 이 보고서가 §2.1에서 인용한 위협 모델링 논문의 지적에 직접 대응한다.

PR #2099는 서명된 Agent Card의 검증 절차를 고친다. 현재 스펙 8.4.3절 2단계는 "검증자가 서명자가 제공한 `jku` 헤더로 서명 키를 가져오도록 지시하여, 서명자가 자기 신뢰 루트를 스스로 통제하게 한다(CWE-863)". 수정안은 키 탐색을 "검증자 측 정책(설정된 JWKS 엔드포인트, 신뢰 키 저장소, 또는 검증자가 통제하는 경로)"으로 제한하고 `kid`는 그렇게 등록된 키 중 고르는 데만 쓰게 한다[^s40]. 서명된 Agent Card는 1.0의 대표 기능이었는데[^s03][^s37], 그 서명의 신뢰 루트를 서명자가 정할 수 있었다는 뜻이다. 4월 개정판이 "서명된 Agent Card가 논문 지적을 부분 해소한다"고 쓴 문장은 이 PR이 병합돼야 온전히 성립한다.

PR #2230은 발견 메커니즘 자체를 바꾸려 한다. "`/.well-known/agent-card.json` 단일 에이전트 발견 메커니즘을 AI Catalog(`/.well-known/ai-catalog.json`)로 대체"하자는 것으로, 근거는 "단일 well-known URI 방식은 도메인당 에이전트 하나에만 작동"한다는 것이다. 새 방식은 다중 에이전트, 다중 테넌트, `entry.data`를 통한 Agent Card 인라인 임베딩, 임의 URL의 인증된 테넌트별 카탈로그 엔드포인트를 지원한다[^s41]. 이 사이트의 KYA 보고서가 "에이전트 신원의 전역 유일성"을 미해결로 꼽았는데, 도메인당 하나라는 제약이 그 문제의 한 단면이었던 셈이다.

두 PR 모두 열린 상태다. 스펙 버전이 4개월째 같다는 사실과 "아무 일도 없다"는 것은 다르다.

**SDK: 스펙보다 빠르게, 서로 다른 속도로.** JS SDK는 9월 18일 v1.2.0을, Python SDK는 9월 21일 v1.1.5를 냈다[^s38][^s39]. 두 SDK의 마이너 버전이 어긋난 채 각자 움직인다. JS 릴리스는 버그 수정 12건으로, 확장 Agent Card 서명 검증 캐싱, 태스크 상태 전이 가드와 원자적 `cancelTask`, JSON-RPC 테넌트 요청 컨텍스트 보존, 잘못된 태스크 ID·상태 필터 거부, 푸시 알림 설정 처리가 들어 있다[^s38]. Python은 protobuf 7 지원과 "범위를 벗어난 datetime 태스크 정렬 회피"가 핵심이다[^s39]. 4월 개정판이 지적했던 `DefaultRequestHandler` 행 버그 계열의 작업이 아직 진행 중이라는 뜻으로 읽힌다.

**재단: A2A가 "창립 프로젝트"로 불리기 시작했다.** AAIF는 9월 15일 Gates 재단을 "Agentic AI Foundation에 가입한 최초의 자선 조직"으로 맞았고, Matt Vasey가 이사회 의석을 얻었다[^s43]. 그런데 같은 보도자료의 정형 문구가 눈에 띈다. "MCP, A2A, AGENTS.md, goose, agentgateway, Agent Router를 **창립 프로젝트**로 하여 AAIF는 핵심 표준과 프로토콜을 관리한다"[^s43]. 한 달 전인 8월 13일 보도자료의 같은 자리에는 "MCP, goose, AGENTS.md, agentgateway"만 있고 A2A는 없다[^s42]. A2A는 2026년 8월에 가입했고 AAIF는 2025년 12월에 출범했으므로, 9월 문구의 "창립"은 부정확하다. 사소해 보이지만 방향이 있다. 가입 5주 만에 A2A가 재단의 정체성 문구 안으로 들어갔다.

같은 기간 재단 자체도 형태를 바꿨다. 9월 1일 운영 이사회와 기술 위원회가 Growth 아래에 **Sandbox** 단계를 신설했다. 요건은 "작동하는 구현에 더해 초기 외부 관심 또는 그럴듯한 논거", 그리고 "이름이 명시된 활동 중인 메인테이너"이며, 받는 것은 "표준 인프라뿐. 자금도, 마케팅도, 스캐닝도 없다". 졸업까지 12개월이 주어진다[^s44]. 대조적으로 Growth는 "서로 무관한 두 조직의 프로덕션 사용, 6개월간 둘 이상 조직의 커밋"을 요구한다[^s44]. A2A가 들어간 Growth 단계가 무엇을 뜻하는지가 이 문서로 처음 수치화됐다. AAIF 자신은 이 스택을 "MCP: 에이전트-도구 연결, A2A: 에이전트-에이전트 상호운용"으로 나눠 설명한다[^s45]. Agent Router(구 Envoy AI Gateway)도 새로 합류해 §2.1의 그림 A는 이제 프로젝트가 더 늘어난 상태다.

회원 수도 8월 13일 기준 247곳으로, 신규 57곳에 Visa·Wells Fargo·Alibaba(Gold)와 NHN KCP·Coocon·Galaxia Moneytree(Silver), ETRI(Associate) 같은 금융·APAC 조직이 들어왔다[^s42].

**바뀌지 않은 것.** A2A 프로젝트 블로그는 8월 27일 이후 새 글이 없다[^s28]. 채택 수치는 여전히 "150개 이상 조직"이고 출처는 프로젝트 자신이다. 독립적인 운영 postmortem은 이번 검색에서도 나오지 않았다.
...
본 리포트는 공개 자료를 기준으로 작성되었으므로, 비공개 enterprise deployment의 실제 SLA, 장애율, 비용, 내부 운영 프로세스는 직접 검증하지 못했다. 특히 `2026-09-22` 재검증 시점에도 채택 신호("150개 이상")의 출처는 프로젝트 자신이며[^s28], 강한 채택 신호 중 상당수는 Linux Foundation 또는 플랫폼 벤더가 직접 공개한 자료에 기반하므로, 이를 독립 감사 수준의 증거와 동일시하면 안 된다.[^s15][^s16][^s17] 또한 TCK, Inspector, SDK, 이슈 트래커를 통해 성숙도를 어느 정도 평가할 수는 있지만, 이 역시 공개 저장소에 드러난 면만 본 것이다.[^s19][^s20][^s21][^s22][^s23][^s24][^s25]

따라서 이 문서는 A2A를 "실전 검토 가능한 프로토콜"로 평가하지만, 동시에 공개 evidence의 성격을 감안해 일부 결론은 `early signal` 또는 `vendor-stated` 수준으로 읽어야 한다고 본다. 특히 독립적인 장문 운영 사례와 실패 postmortem이 아직 많지 않다는 점은 중요한 한계다.[^s15][^s26][^s27] 9월 재검증에서 독립 학술 분석은 두 편 확보했지만[^s34][^s35], 운영 postmortem은 여전히 없다. 9월 22일 재검증에서는 열린 PR 두 건(#2099, #2230)을 근거로 삼았는데, 이는 병합되지 않은 제안이므로 현재 스펙의 서술이 아니라 진행 중인 수정으로 읽어야 한다. AAIF 편입 관련 Forbes·Axios 원문은 403으로 읽지 못해 Yahoo Tech 전재본과 프로젝트 공지에 의존했다.

## 결론

Google A2A는 현재 가장 유력한 에이전트 상호운용 프로토콜 후보 중 하나다. 공식 스펙은 `1.0.1`에서 4개월째 멈춰 있고, 2026년 8월부터 MCP와 같은 재단(AAIF) 아래 있으며, SDK는 스펙을 앞질러 각자 움직인다.[^s28][^s30][^s36][^s37] 여기에 2026년 시점의 플랫폼 통합과 production-use 발표까지 더해지면서, 적어도 "문서 속 아이디어" 단계는 확실히 넘어섰다고 볼 수 있다.[^s15][^s16][^s17] 특히 `Agent Card + Task + Artifact + multi-binding` 조합은 단순 도구 호출로는 다루기 어려운 상태ful 협업 문제를 상당히 잘 모델링한다.[^s01][^s03]
